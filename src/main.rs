...
fn pack(input_dir: PathBuf, output_dir: PathBuf, name: &String, extension: &String, package_size: u64, _compress: bool) {
    ...
    progress.set_style(
        ProgressStyle::with_template("{spinner:.green} [{elapsed_precise}] [{wide_bar:.cyan/blue}] Packing: {pos}/{len}")
    ...
    let mut package_file = fs::File::create(
        output_dir.join(format!("{name}.{:03}.{extension}", package_index))
    ).unwrap();
...
fn main() {
    ...
    match args.command {
        Commands::Pack { 
            ...
            compress, 
            input_dir 
        } => pack(input_dir, output_dir, &package_name, &package_extension, package_size, compress),
        _ => println!("{:?}", args)
    }
}